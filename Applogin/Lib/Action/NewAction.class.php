...
class NewAction extends CommonAction{
	public function index(){
		...
		$this->show=$page->show();
		$this->news=$db->field('id,pid,title,sort')->order('sort asc,id desc')->limit($page->firstRow.','.$page->listRows)->select();
	...
	public function mod(){
		$this->list=recursive(M('List')->field('id,pid,name,type')->where("type='new'")->order('sort')->select());
		$this->news=M('New')->field('id,pid,sort,url,title,keywords,description,contents')->find($this->_get('id','intval'));
		$this->display();	
	}
