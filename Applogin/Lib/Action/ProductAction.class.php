...
class ProductAction extends CommonAction{
	public function index(){
		...
		$this->show=$page->show();
		$this->product=$db->field('id,pid,name,sort,thumb,featured')->order('sort asc,id desc')->limit($page->firstRow.','.$page->listRows)->select();
	...
	public function mod(){
		$this->list=recursive(M('List')->field('id,pid,name,type')->where("type = 'product'")->order('sort')->select());
		$product=M('Product')->field('id,pid,sort,name,property1,property2,property3,property4,url,title,keywords,description,thumb,photo,contents,featured')->find($this->_get('id','intval'));
